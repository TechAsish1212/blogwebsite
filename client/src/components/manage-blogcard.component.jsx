...
const BlogStats = ({ stats }) => {
    return (
        <div className='flex gap-2 max-lg:mb-6 max-lg:pb-6 border-grey max-lg:borber-b'>
            {
            ...
            <div className='flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center'>
                ...
                <div className='flex flex-col justify-between py-2 w-full min-w-[300px]'>
                    ...
                    <div className='flex gap-6 mt-5'>
                        <Link to={`/editor/${blog_id}`} className='pr-4 py-2 text-black' title='Edit'><i className="fi fi-rr-file-edit text-2xl"></i></Link>

                        <button className='lg:hidden pr-4 py-2 text-black' title='Stats' onClick={() => setShowStat(preVal => !preVal)}><i className="fi fi-sr-stats text-2xl"></i></button>

                        <button className='pr-4 py-2  text-red' title='Delete' onClick={(e) => deleteBlog(blog, access_token, e.target)}><i className="fi fi-rr-trash text-2xl"></i></button>
                    </div>
                </div>
        ...
        <div className='flex gap-5 lg:gap-10 pb-6 border-b mb-6 border-grey'>
            ...
            <div>
                ...
                <div className='flex gap-6 mt-3'>
                    ...
                    <button className='pr-4 py-2  text-red' title='Delete' onClick={(e) => deleteBlog(blog,
                        access_token, e.target)}><i className="fi fi-rr-trash text-2xl"></i></button>
                </div>
            </div>
        </div>
    )
